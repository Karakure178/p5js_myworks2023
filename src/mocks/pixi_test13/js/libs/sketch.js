...
import { AppClick } from './event/appclick';
...
export class Sketch {
  ...
  _init() {
    ...
    // テストコード
    const conteiner = new PIXI.Container();

    const path = '../../mocks/pixi_test13/images/test1.png';
    PIXI.Assets.load(path).then((texture) => {
      ...
      this.app.stage.addChildAt(img, 0);
    });

    // テストコード(shader)
    new Grid({ app: this.app, container: conteiner });
    this.app.stage.addChild(conteiner);
    new AppClick(this.app, conteiner).addEvent();
  }
